...
### Review repairs — model v4 and storage transport

- `heuristic-v4` models supported negative price outcomes using complement probabilities before the digital/touch formula. Unsupported scope, multiple conditions and double negatives are excluded; finite-difference sensitivity uses log returns to preserve complement symmetry. Existing tail/expiry safeguards remain.
- Fingerprints include effective question text, scoring version, expiry, outcome token and sorted mappings. Preserve all old observations/mappings. Earlier `heuristic-v3` archives span multiple parser behaviors, so the old identifier alone cannot reproduce every historic result. No relabeling or historical backfill is performed.
- Deploy Worker additions **before** the Vercel collector. `/internal/payload/startup` returns the fixed startup metadata set as raw JSON values; collector-only PUT operations `latest`, `catalog`, `snapshot`, `mapping` accept bounded JSON text, validated in D1. Archive coordinates are bounded query parameters. The old SQL gateway remains for small operations and rollback. No migration, binding, secret, cron or plan change is required.
- The private read-only `/snapshot/raw` route forwards stored JSON to Vercel, whose existing `freshResearchSnapshot` still removes stale rows on every read/cache hit. The legacy `/snapshot` route retains Worker-side filtering for older deployments.
- History projects requested fields in D1. Updated app/export clients request `limit=100`; the Vercel six-hour endpoint follows all cursors and rejects incomplete pagination. The legacy omitted-limit behavior remains 500 for compatibility with old deployments. Unknown signals produce empty maps, not data from other signals.
